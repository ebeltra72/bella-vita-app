...
const API = {
  async getVisitas() {
    ...
    return rows.map(v => ({
      ...
      checkin: v.checkin,
      checkout: v.checkout,
      latCheckin: v.lat_checkin,
      lngCheckin: v.lng_checkin,
      latCheckout: v.lat_checkout,
      lngCheckout: v.lng_checkout,
      distCheckin: v.dist_checkin,
      distCheckout: v.dist_checkout,
      gpsOkCheckin: v.gps_ok_checkin,
      gpsOkCheckout: v.gps_ok_checkout,
      simulado: v.simulado,
...
function useGPS() {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (e) => {
          ...
          if (e.code === 1 || e.message?.toLowerCase().includes("permission")) {
            setSinPermiso(true); rej(e);
          } else {
            setError(e.message); rej(e);
          }
        },
  ...
  const simular = (sucursal) => ({
    latitude:  (sucursal?.lat ?? -34.6037) + (Math.random() - 0.5) * 0.001,
    longitude: (sucursal?.lng ?? -58.3816) + (Math.random() - 0.5) * 0.001,
    simulado: true,
  });
...
function VistaAdrian({ sucursales, preguntas, visitas, setVisitas }) {
  ...
  const handleCheckin = async () => {
    try { registrarCheckin(await obtener()); } catch {}
  };
  ...
  const registrarCheckout = async (coords) => {
    ...
    try {
      ...
      // igual guardamos local si falla la API
      setVisitas((p) => [fin, ...p]);
    }
    setGuardando(false);
  ...
  const handleCheckout = async () => {
    try { await registrarCheckout(await obtener()); } catch {}
  };
    ...
    <div style={s.section}>
      <div style={s.card}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
          ...
          {visitaActual?.simulado && <span style={s.badge("amarillo")}>simulado</span>}
        </div>
      ...
      {sinPermiso ? (
        <div style={{ ...s.card, background: C.amarilloL, border: `1px solid ${C.amarillo}` }}>
          <div style={{ fontWeight: 700, fontSize: 13, color: C.amarillo, marginBottom: 6 }}>⚠ GPS sin permiso</div>
          <div style={{ fontSize: 12, color: C.texto, marginBottom: 12 }}>En el celu real el GPS funciona. Podés simular para probar.</div>
          <button style={s.btn("ghost")} onClick={() => { setSinPermiso(false); registrarCheckout(simular(sucursal)); }}>📍 Simular Check-out</button>
        </div>
      ) : (
        <button style={s.btn("danger")} onClick={handleCheckout} disabled={loading || guardando}>
      ...
      {sinPermiso ? (
        <div style={{ ...s.card, background: C.amarilloL, border: `1px solid ${C.amarillo}` }}>
          ...
          <button style={{ ...s.btn("ghost"), opacity: !sucursalId ? 0.5 : 1 }}
            disabled={!sucursalId} onClick={() => { setSinPermiso(false); registrarCheckin(simular(sucursal)); }}>
            📍 Simular Check-in
          </button>
        </div>
      ) : (
...
function VistaEquipo({ equipo, registros, setRegistros }) {
  ...
  const guardar = async () => {
    ...
    const nuevo = {
      id: yaCargoHoy ? registroHoy.id : Date.now(),
      personaId: persona, personaNombre: miembro.nombre,
      fecha: fechaHoy,
      mensajes: Number(form.mensajes), turnos: Number(form.turnos), senias: Number(form.senias),
      nota: form.nota, editado: yaCargoHoy ? new Date().toISOString() : null,
    };
    setGuardando(true);
    try {
      await API.saveRegistro(nuevo);
      setRegistros((prev) => {
  ...
  const editar = () => {
    setForm({ mensajes: String(registroHoy.mensajes), turnos: String(registroHoy.turnos), senias: String(registroHoy.senias), nota: registroHoy.nota || "" });
    setGuardado(false);
  };
    ...
    <div style={s.section}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {equipo.map((m) => (
            <button key={m.id} onClick={() => { setPersona(m.id); setGuardado(false); setForm({ mensajes: "", turnos: "", senias: "", nota: "" }); }} style={{
              ...
              border: `2px solid ${persona === m.id ? C.violeta : "#E5E7EB"}`,
      ...
      {persona && yaCargoHoy && !guardado && (
        <div style={{ ...s.card, background: C.violetaL, border: `1px solid ${C.violeta}` }}>
          <div style={{ color: C.violeta, fontWeight: 700, fontSize: 13, marginBottom: 10 }}>✓ Ya cargaste tus números hoy</div>
      ...
      {persona && (!yaCargoHoy || guardado === false) && !yaCargoHoy && (
        <div style={s.card}>
          <div style={{ fontWeight: 700, fontSize: 14, color: C.violeta, marginBottom: 14 }}>
            {`Números de hoy — ${new Date().toLocaleDateString("es-AR", { weekday: "long", day: "numeric", month: "long" })}`}
          </div>
          ...
          <button style={{ ...s.btn("purple"), opacity: (!form.mensajes || !form.turnos || !form.senias || guardando) ? 0.5 : 1 }}
            disabled={!form.mensajes || !form.turnos || !form.senias || guardando}
            onClick={guardar}>
...
function GestionComercialPanel({ registros, equipo, meta, setMeta }) {
  ...
  const totalesPorPersona = equipo.map((m) => {
    ...
    return { ...m, ...tot, pctSenias, diasCargados: regs.length, premio: seniasExtra * meta.premioPorSenia };
  });
  ...
  return (
    <div style={s.section}>
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        {[["resumen", "📊 Resumen"], ["detalle", "📋 Detalle"], ["metas", "🎯 Metas"]].map(([t, l]) => (
          <button key={t} onClick={() => setSubTab(t)} style={{
            ...
            border: `2px solid ${subTab === t ? C.violeta : "#E5E7EB"}`,
      ...
      {subTab === "resumen" && totalesPorPersona.map((p) => {
        const bars = [["💬 Mensajes", p.mensajes, meta.mensajes], ["📅 Turnos", p.turnos, meta.turnos], ["🤝 Señas", p.senias, meta.senias]];
        return (
          <div key={p.id} style={s.card}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
              ...
              <span style={s.badge(p.senias >= meta.senias ? "purple" : "amarillo")}>{p.senias >= meta.senias ? "✓ Meta" : "En progreso"}</span>
            </div>
            {bars.map(([lbl, val, met]) => {
              const pct = met > 0 ? Math.min(100, (val / met) * 100) : 0;
              return (
                <div key={lbl} style={{ marginBottom: 10 }}>
                  ...
                  <div style={{ background: C.grisL, borderRadius: 99, height: 7, overflow: "hidden" }}>
                    <div style={{ height: "100%", borderRadius: 99, width: `${pct}%`, background: pct >= 100 ? C.violeta : pct >= 60 ? C.verdeL : C.amarillo }} />
                  </div>
                </div>
              );
            })}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 12 }}>
              ...
              <div style={{ background: p.premio > 0 ? C.violetaL : C.grisL, borderRadius: 10, padding: "10px 12px" }}>
                ...
                <div style={{ fontSize: 18, fontWeight: 800, color: p.premio > 0 ? C.violeta : C.gris }}>${p.premio.toLocaleString("es-AR")}</div>
              </div>
            </div>
          </div>
        );
        ...
        <div style={s.card}>
          ...
          {[["mensajes", "💬 Meta mensual de mensajes"], ["turnos", "📅 Meta mensual de turnos"], ["senias", "🤝 Meta mensual de señas"], ["premioPorSenia", "💰 Premio por seña sobre meta ($)"]].map(([k, l]) => (
            <div key={k} style={{ marginBottom: 14 }}>
              <span style={s.label}>{l}</span>
              <input type="number" min="0" style={s.input} value={meta[k]} onChange={(e) => setMeta((m) => ({ ...m, [k]: Number(e.target.value) }))} />
            </div>
          ))}
          <div style={{ ...s.card, background: C.violetaL, padding: "12px 14px", marginBottom: 0 }}>
            <div style={{ fontSize: 12, color: C.violeta, lineHeight: 1.5 }}>
              <strong>Cómo funciona el premio:</strong> cada seña que supere la meta mensual suma ${meta.premioPorSenia.toLocaleString("es-AR")}. El resultado se muestra en el resumen por persona.
            </div>
          </div>
        </div>
      )}
      ...
      {filas.map((v) => (
        <div key={v.id} style={{ ...s.card, padding: "11px 13px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            {v.km != null ? <span style={s.badge("verde")}>{v.km} km</span> : <span style={{ fontSize: 12, color: C.gris }}>origen</span>}
          </div>
        </div>
      ))}
    </div>
...
export default function App() {
  ...
  const VISTAS_HEADER = [["adrian", "🗺 Adrián"], ["equipo", "💼 Equipo"], ["ileana", "👩‍💼 Panel Ileana"]];
  const TABS_ILEANA = [["historial", "📋 Visitas"], ["gestion", "💜 Comercial"], ["encuesta", "📝 Encuesta"], ["sucursales", "📍 Sucursales"], ["km", "🚗 Km"]];
  ...
  return (
    <div style={s.app}>
      ...
      {cargando && (
        <div style={{ textAlign: "center", padding: 40, color: C.gris }}>Cargando datos…</div>
      )}

      {!cargando && vista === "adrian" && (
        <VistaAdrian sucursales={sucursales} preguntas={preguntas} visitas={visitas} setVisitas={setVisitas} />
      )}
      {!cargando && vista === "equipo" && (
